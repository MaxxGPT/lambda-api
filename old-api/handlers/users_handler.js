            ...
            User.findOne({
              ...
              if (err) {
                ...
                if (
                  ...
                  _userFromModel.history.push({
                    field: "password",
                    value: _userFromModel.password,
                  });
                }
                User.findOneAndUpdate(
                  {
      ...
      tokenMiddleware.validateToken(
        ...
        (err, data) => {
          if (err) {
            ...
            let cognitoidentityserviceprovider =
              new AWS.CognitoIdentityServiceProvider();
            User.findById(event.pathParameters.id).exec(
              (err, _userFromModel) => {
                if (err) {
                  callback(null, {
                    statusCode: err.statusCode || 500,
                  ...
                  if (_userFromModel.name !== body.name) {
                    _userFromModel.history.push({
                      field: "name",
                      value: _userFromModel.name,
                    });
                  }
                  if (_userFromModel.email !== body.email) {
                    _userFromModel.history.push({
                      field: "email",
                      value: _userFromModel.email,
                    });
                  }
                  User.findOneAndUpdate(
                    {
        ...
        if (event.userPoolId == process.env.COGNITO_USER_POOL) {
            if (event.triggerSource == 'PostConfirmation_ConfirmSignUp') {
                ...
                let newUser = new User({
                    ...
                    plan: mongoose.Types.ObjectId('63f11ba3b1ab44c5ae9f3bfd')
                });
